...
public class Discovery {
	...
	private final Map<String, Set<URI>> announceInfo;
	/**
	...
	public Discovery(InetSocketAddress addr, String serviceName, String serviceURI) throws SocketException, UnknownHostException, IOException {
		...
		this.announceInfo = new ConcurrentHashMap<>();
	...
	public void start() {
		...
		if (this.serviceName != null && this.serviceURI != null) {
			...
			try {
				// start thread to send periodic announcements (fazer duas classes diferentes para o trabalho)
				new Thread(() -> {
					for (;;) {
						try {
							ms.send(announcePkt);
			...
			for (;;) {
				try {
					...
					if (msgElems.length == 2) { // periodic announcement
                        ...
                        announceInfo.computeIfAbsent(serviceName,
								k -> ConcurrentHashMap.newKeySet()).add(uri);
					}
				} catch (IOException e) {
					// do nothing
				}
	...
	public URI[] knownUrisOf(String serviceName, int minReplies) throws InterruptedException {
		...
		while(true) {
			Set<URI> uris = announceInfo.getOrDefault(serviceName, Collections.emptySet());
